/*
 * LAB2: Program to flash call and flash the FIR code to the STM board
 *
 * An input waveform is copied to the output DAC. The output coefficients
 * from the ece486_FIR()   to the second DAC output.  Each
...
// initialize number of coefficients and their values
float h[20];
for(j = 0; j < 20; j++) {
	h[j] = .05;
}
